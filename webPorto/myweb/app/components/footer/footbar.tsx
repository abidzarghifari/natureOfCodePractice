
export default function Footbar(){

    return (
        <div>
            
            <div className="h-120 sticky bg-foreground inset-x-0 bottom-0 p-5 md:px-20 md:py-20">

                <div className="grid lg:grid-cols-5">
                    <div className="text-background col-span-2">
                        <p>Contact Me</p>
                        <h1 className="text-6xl py-10">Let's start creating together!</h1>
                    </div>
                    <div className="col-span-3 grid grid-cols-2">
                        <div>
                                     
                        </div>
        
                        <div className="flex flex-col gap-5 space-x-6 mt-4 md:mt-0">
                            <a href="#" className="group flex items-center space-x-2 text-background hover:text-muted-foreground transition-colors duration-300">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" stroke-linejoin="round" className="lucide lucide-instagram"><rect width="20" height="20" x="2" y="2" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" x2="17.51" y1="6.5" y2="6.5"/></svg>
                                <span className="text-xl font-medium hidden sm:block">Instagram</span>
                            </a>

                            <a href="#" className="group flex items-center space-x-2 text-background hover:text-muted-foreground transition-colors duration-300">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-github"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"/><path d="M9 18c-4.51 2-5-2-7-2"/></svg>
                                <span className="text-xl font-medium hidden sm:block">GitHub</span>
                            </a>

                            <a href="#" className="group flex items-center space-x-2 text-background hover:text-muted-foreground transition-colors duration-300">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-linkedin"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect width="4" height="12" x="2" y="9"/><circle cx="4" cy="4" r="2"/></svg>
                                <span className="text-xl font-medium hidden sm:block">LinkedIn</span>
                            </a>

                            <a href="#" className="group flex items-center space-x-2 text-background hover:text-muted-foreground transition-colors duration-300">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-twitter"><path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z"/></svg>
                                <span className="text-xl font-medium hidden sm:block">Twitter</span>
                            </a>
                        </div>

                    </div>
                </div>

                <div className="py-10">
                    <p className="text-muted-foreground">2026 develop with passion by Dikayoda</p>
                </div>

            </div>
        </div>  
    );
}